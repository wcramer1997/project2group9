...
tribes <- creatures %>%
  filter(
    subtypes == "Zombie" | subtypes == "Human" | subtypes == "Goblin" | subtypes == "Beast" | subtypes == "Wurm" | subtypes == "Angel" | subtypes == "Leviathan"
  ) %>%
  mutate(
...
tribes_size +
  scale_color_manual(values = c("orchid2", "darkgreen", "red", "lemonchiffon2", "royalblue4",  "greenyellow", "black")) +
  theme(
  panel.background = element_rect(fill = "lightblue",
                                colour = "lightblue",
                                size = 0.5, linetype = "solid"),
...
filter(
  subtypes == "Zombie" | subtypes == "Goblin" | subtypes == "Beast" | subtypes == "Wall") %>%
group_by(
...
ggplot(power_to_toughness_by_tribe) +
  geom_point(aes(
    x = toughness,
    y = power,
    size = num_cases,
    color = subtypes),
    alpha = .5 ) +
  scale_color_manual(values = c("forestgreen", "darkred", "tan4", "black")) +
